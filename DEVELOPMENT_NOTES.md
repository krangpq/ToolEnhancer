...
### 버그 #1: 강화 중 아이템 이동 시 복제 발생 ⚠️ 치명적

**발생 버전**: v1.0.4 이전

**문제 상황**:
```
1. 플레이어가 손에 든 검으로 /enhance 실행
2. GUI가 열리고 session.targetItem에 검의 복제본 저장
3. 플레이어가 GUI 열린 상태에서 원본 검을 다른 슬롯으로 이동
4. 강화 버튼 클릭 → session.targetItem(복제본)으로 강화 진행
5. 강화 성공 시 원본 검 + 강화된 검 = 아이템 복제!
```

**근본 원인**:
- `session.targetItem`은 GUI 열 때의 **스냅샷(복제본)**일 뿐
- 실제 플레이어 인벤토리와 **동기화되지 않음**
- 강화 실행 시점에 원본 아이템의 위치/존재 여부를 검증하지 않음

**해결 방법** (v1.0.5):
```java
// EnhanceGUI.java - GuiSession 클래스에 originalSlot 추가
class GuiSession {
    final ItemStack targetItem;    // 복제본 (참조용)
    final int originalSlot;        // ⭐ 원본 위치 추적
}

// handleEnhanceButtonClick() 에서 강화 실행 전 검증
...
if (originalItem == null || !isSimilarItem(originalItem, session.targetItem)) {
    player.sendMessage(ChatColor.RED + "아이템이 이동되었거나 변경되었습니다!");
    player.closeInventory();
    return;
}

// 검증 통과 후 originalItem (실제 아이템)으로 강화 진행
EnhanceResult result = enhanceManager.performEnhance(
    originalItem.clone(), // ⭐ 복제본이 아닌 실제 아이템 사용
    session.selectedEnchantment, 
    stoneCount, 
    stoneManager
);
...
} else if (result.getType() == DESTROYED) {
    player.getInventory().setItem(session.originalSlot, new ItemStack(Material.AIR));
}
```

**관련 코드**:
- `EnhanceGUI.java:383-423` (handleEnhanceButtonClick)
- `EnhanceGUI.java:528-550` (GuiSession 클래스)
- `EnhanceGUI.java:557-595` (isSimilarItem 메서드)

**교훈**: GUI에서 다루는 아이템은 항상 "참조"일 뿐이며, 실제 작업은 원본 인벤토리를 직접 검증하고 수정해야 함

---

### 버그 #2: 강화 버튼 중복 클릭으로 인한 강화석 중복 소모 ⚠️ 중요

**발생 버전**: v1.0.5 이전

**문제 상황**:
```
1. 플레이어가 빠르게 강화 버튼을 여러 번 클릭
2. 첫 번째 클릭 처리 중 두 번째 클릭도 접수됨
3. 강화석이 2배로 소모되거나 중복 강화 시도 발생
```

**근본 원인**:
- GUI 클릭 이벤트는 **비동기적으로 발생** 가능
- 강화 로직 실행 중에도 새로운 클릭 이벤트를 받을 수 있음
- 실행 중 상태를 추적하는 메커니즘 없음

**해결 방법** (v1.0.6):
```java
// EnhanceGUI.java - 클래스 변수로 처리 중인 플레이어 추적
private final Set<UUID> processingPlayers = ConcurrentHashMap.newKeySet();

// handleEnhanceButtonClick() 시작 부분
...
try {
    ...
    // ⭐ 반드시 finally에서 제거 (예외 발생해도 락 해제)
    processingPlayers.remove(player.getUniqueId());
}
```

**관련 코드**:
- `EnhanceGUI.java:33` (processingPlayers 선언)
- `EnhanceGUI.java:383-423` (handleEnhanceButtonClick의 try-finally)

**교훈**: 비용이 드는 작업(강화석 소모)은 반드시 중복 실행 방지 장치 필요

---

### 버그 #3: GUI 닫을 때 강화석이 사라지는 문제 ✅ 경미

**발생 버전**: v1.0.4 이전

**문제 상황**:
```
1. 플레이어가 강화석을 GUI에 배치
2. ESC 키로 GUI 닫기
3. 강화석이 반환되지 않고 사라짐
```
...
**해결 방법**:
```java
// EnhanceGUI.java - onInventoryClose 이벤트 핸들러
@EventHandler
public void onInventoryClose(InventoryCloseEvent event) {
    if (title.equals(ChatColor.DARK_GREEN + "도구 강화")) {
        // 처리 중이 아닐 때만 반환 (성공/실패 후에는 반환 안함)
...
**관련 코드**:
- `EnhanceGUI.java:475-487` (onInventoryClose)
- `EnhanceGUI.java:492-507` (returnEnhanceStones)

---

### 버그 #4: 대상 아이템 이동 시도 차단 로직 ⚠️ 중요

**추가된 버전**: v1.0.6

**문제 인식**:
```
버그 #1을 수정했지만, 플레이어가 GUI 열린 상태에서 
대상 아이템을 이동하려고 시도하면 혼란스러운 상황 발생
→ 아예 이동 자체를 막는 것이 더 직관적
```

**해결 방법**:
```java
// handleEnhanceProcessClick() - 플레이어 인벤토리 영역 클릭 시
if (slot >= 54) { // 플레이어 인벤토리 영역
    // 1. 클릭한 아이템이 대상 아이템인지 확인
...
**관련 코드**:
- `EnhanceGUI.java:308-338` (handleEnhanceProcessClick의 이동 차단 로직)

**교훈**: 버그를 막는 것도 중요하지만, 사용자가 실수할 가능성 자체를 원천 차단하는 것이 더 좋음
