...
public class EnhanceStoneManager {
    ...
    public EnhanceStoneManager(Plugin plugin) {
        ...
        try {
            ...
            plugin.getLogger().severe("설정 로드 중 오류 발생!");
            plugin.getLogger().severe("에러: " + e.getMessage());
            plugin.getLogger().severe("=========================================");
            e.printStackTrace();
            throw new RuntimeException("EnhanceStoneManager 초기화 실패", e);
        }
    }
     ...
     *
     * @param amount 생성할 개수
     * @return 강화석 ItemStack
     */
    public ItemStack createEnhanceStone(int amount) {
        if (amount <= 0) {
            plugin.getLogger().warning("createEnhanceStone() 호출 오류: amount가 0 이하입니다! (" + amount + ")");
            amount = 1;
        }
        ...
        try {
            ...
            plugin.getLogger().severe("강화석 생성 중 오류: " + e.getMessage());
            e.printStackTrace();
     ...
     * @param baseRate 기본 성공률 (0.0~1.0)
     * @return 최종 성공률 (0.0~1.0)
     */
    public double calculateSuccessRate(int stoneCount, double baseRate) {
        if (stoneCount <= 0) {
            plugin.getLogger().warning("calculateSuccessRate() 호출 오류: stoneCount가 0 이하입니다!");
            return 0.0;
        }
        ...
        try {
            ...
            plugin.getLogger().severe("성공률 계산 중 오류: " + e.getMessage());
            e.printStackTrace();
            return 0.0;
        }
    }
    ...
    public double calculateDestroyRate(int stoneCount, int level) {
        ...
        try {
            ...
            plugin.getLogger().severe("파괴율 계산 중 오류: " + e.getMessage());
            e.printStackTrace();
            return 0.0;
        }
    }
     ...
     * @param maxLevel 해당 인챈트의 바닐라 최대 레벨
    ...
    public double getBaseSuccessRate(int currentLevel, int maxLevel) {
        try {
            ...
            plugin.getLogger().severe("기본 성공률 계산 중 오류: " + e.getMessage());
            e.printStackTrace();
            return 0.05; // 안전한 기본값
        }
    }
     ...
     *
     * @param vanillaMaxLevel 바닐라 최대 레벨
     * @return 실제 최대 레벨
     */
    public int getAbsoluteMaxLevel(int vanillaMaxLevel) {
        try {
            ...
            boolean limitEnabled = plugin.getConfig().getBoolean(
                    "success_rates.beyond_vanilla.max_level_limit.enabled", false);
            ...
            plugin.getLogger().severe("최대 레벨 계산 중 오류: " + e.getMessage());
            e.printStackTrace();
            return vanillaMaxLevel * 2; // 안전한 기본값
        }
    }
    ...
    public void registerRecipes() {
        try {
            ...
            plugin.getLogger().severe("=========================================");
            plugin.getLogger().severe("강화석 레시피 등록 실패!");
            plugin.getLogger().severe("에러: " + e.getMessage());
            plugin.getLogger().severe("=========================================");
            e.printStackTrace();
        }
